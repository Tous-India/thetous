  ...
  try {
    ...
    try {
      await sendMetaEvent({
        ...
        customData: { form_type: "contact", services: safeServices },
        eventSourceUrl,
        clientIp,
        clientUserAgent,
      });
    } catch (capiErr) {
      console.warn("[contact] CAPI Lead event failed:", capiErr?.message || capiErr);
    }

    return NextResponse.json(
      { message: "Email sent successfully", eventId },
      { status: 200 }
    );
  } catch (error) {
